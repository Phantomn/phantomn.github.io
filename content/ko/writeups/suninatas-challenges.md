...
처음엔 mp3 파일에 스테가노그래피로 다른 파일을 숨겼을 거라 생각했지만, 기초 분석을 건너뛰고 넘겨짚었던 케이스였다.

파일 속성만 확인해봐도 바로 플래그가 있었다.
...
카나리가 없어 보인다. IDA가 인식을 못 한 건지, 실제로 빠진 건지는 확인이 더 필요하다.
...
처음에 접근을 잘못 잡았다 — 이 문제는 input에 0xdeadbeef가 들어 있어야 한다.
...
에러가 발생한다. 이유는 leave 명령 시 ebp를 esp에 가져오고 +4하기 때문이다.
